//###########################################################################
//    __                    ____            __
//   / /  __ __ ___ _  __  / __/___  ____  /_/___  ____
//  / /__/ // /  _ \ \/ / / __/  _ \/ __ \/ /  _ \/  _/
//  \___/__, /__//_/_/\_\ \__/__//_/\__, /_/__//_/\__/
//      /___/                       /___/ 
// 
//  LynxEngine Ver 1.00 
//  Copyright (C) 2003 fallingCAT studios.  All Rights Reserved.
//
//
//  Created by Owen Wu : 2005/11/02
//  Last Update : 
//--------------------------------------------------------------------------
//  說明:
//
//###########################################################################

#ifndef __LYNXOSPTREE_H__
#define __LYNXOSPTREE_H__

#include <LynxSDK.h>
//#include <LynxOSP.h>

#include <LynxObj.h>
#include <SceneSystem/Tree/LynxSceneTree.h>

namespace LynxEngine 
{
	namespace SceneSystem 
	{
		class LYNXENGCLASS CTreeNode : public CObj
		{
		public:
			int						m_ID;	
			LYNXBOUNDARYBOX			m_BBox;		
			LYNXVECTOR3D			m_Center;
			LYNXREAL				m_R;				
		};

		class LYNXENGCLASS COSPNode : public CTreeNode
		{
		public:			
			short					m_lpChildrenID[8];			
		};

		class LYNXENGCLASS COSPLeaf : public CTreeNode
		{
		public:			
			WORD 					m_NumElement;
			LPVOID					m_lpElement;		
		};

		class LYNXENGCLASS COSP : public CTree
		{	
			LYNX_DECLARE_CLASS(COSP);
		protected:
			COSPNode				m_RootNode;		
			DWORD					m_NumNodes;
			DWORD					m_NumLeaves;
			CTreeNode*				m_lpNodeList;
			CTreeNode*				m_lpLeafList;
			LYNXBOOL*				m_lpbLeafVisible;
			
		public:				
			COSP();
			~COSP();

			void									LoadLeaf(LPLYNXOSPLEAF lpn, LPLYNXFILE lpf);
			void									LoadNode(LPLYNXOSPNODE lpn, LPLYNXFILE lpf);
			LYNXBOOL								Load(char *filename);
			LYNXBOOL								Release();

			LYNXINLINE int							GetLeafIndex(LPLYNXVECTOR3D p) {return lynxOSPGetLeafByPosition(&m_OSP, p); };
			LYNXINLINE LYNXOSPLEAF&					GetLeaf(int i) {return m_OSP.lpLeafList[i]; };
			LYNXINLINE LYNXOSPLEAF&					GetLeaf(LPLYNXVECTOR3D p) {return m_OSP.lpLeafList[lynxOSPGetLeafByPosition(&m_OSP, p)]; };

			friend class CScene;
		};
	}
}

#endif

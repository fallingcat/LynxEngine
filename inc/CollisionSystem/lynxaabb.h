...
#ifndef __LYNXAABB_H__
#define __LYNXAABB_H__
...
namespace LynxEngine 
{
	...
	{	
		class LYNXENGCLASS CAABB : public CCollisionObj
		{	
		...
		public:			
			CAABB();
			CAABB(CVector3& c, CVector3& e);
			CAABB(CVector3* p, int Num);
			virtual ~CAABB();	

			LYNXINLINE CAABB& operator =(const CAABB& rhs) 
			{ 
				m_Center = rhs.m_Center; 
				m_Extension = rhs.m_Extension; 
				lynxMemCpy(m_V, rhs.m_V, sizeof(CVector3)*8); 
		...
		LYNXINLINE void	CAABB::SetByBoundary(CVector3& min, CVector3& max) 
		{	
		...
		LYNXINLINE void	CAABB::Set(CVector3* p, int Num) 		
		{
		...
		LYNXINLINE void	CAABB::Set(CVector3* p, DWORD stride, int Num) 		
		{

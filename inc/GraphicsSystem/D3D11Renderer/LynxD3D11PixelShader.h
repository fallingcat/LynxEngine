...
#ifndef __LYNXD3D11PIXELSHADER_H__
#define __LYNXD3D11PIXELSHADER_H__
...
{	
	...
	{		
		class LYNXENGCLASS CD3D11PixelShader : public CPixelShader
		{
			LYNX_DECLARE_CLASS(CD3D11PixelShader);
		protected:			
			static const int				MAX_BOUND_CB = 16;
			ID3D11PixelShader*				m_lpPS;	
			ID3D11ShaderReflection*			m_lpReflection;
			CArray<CD3D11Buffer*>			m_ConstantBufferArray;
			int								m_NumBoundCBs;
			ID3D11Buffer*					m_D3DBufferArray[MAX_BOUND_CB];
		public:						
			CD3D11PixelShader(CGraphicsSystem* lpsys);
			...
			LYNXFORCEINLINE ID3D11PixelShader* GetlpPixelShader() {return m_lpPS; };

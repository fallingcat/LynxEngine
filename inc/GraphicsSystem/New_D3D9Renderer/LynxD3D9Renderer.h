...
#include <GraphicsSystem/New_D3D9Renderer/LynxD3D9Buffer.h>
#include <GraphicsSystem/New_D3D9Renderer/LynxD3D9Texture.h>
#include <GraphicsSystem/New_D3D9Renderer/LynxD3D9Query.h>
#include <GraphicsSystem/New_D3D9Renderer/LynxD3D9VertexLayout.h>
#include <GraphicsSystem/New_D3D9Renderer/LynxD3D9VertexShader.h>
#include <GraphicsSystem/New_D3D9Renderer/LynxD3D9PixelShader.h>
#include <GraphicsSystem/New_D3D9Renderer/LynxD3D9VertexArray.h>
#include <GraphicsSystem/New_D3D9Renderer/LynxD3D9IndexArray.h>
#include <GraphicsSystem/New_D3D9Renderer/LynxD3D9DepthStencilState.h>
#include <GraphicsSystem/New_D3D9Renderer/LynxD3D9BlendState.h>
#include <GraphicsSystem/New_D3D9Renderer/LynxD3D9RasterizerState.h>
#include <GraphicsSystem/New_D3D9Renderer/LynxD3D9SamplerState.h>
...
namespace LynxEngine
{
	...
	{	
		...
		public:						
			...
			LYNXFORCEINLINE WORD					GetVersion(void) {return 100; };			
			LYNXFORCEINLINE const LYNXCHAR*			GetAPIString(void) {return _T("Direct3D 9"); };			
			LYNXFORCEINLINE const LYNXCHAR*			GetTexturePath(void) {return _T(""); };	
			LYNXFORCEINLINE void					SetTexturePath(const LYNXCHAR* path) {};
			...
			LYNXFORCEINLINE void					Start(void) {m_lpD3DDevice->BeginScene(); };			
			LYNXFORCEINLINE void					End(void) {m_lpD3DDevice->EndScene(); };
			...
			LYNXFORCEINLINE void					SwapBackBuffer() {m_lpSwapChain->Present(NULL, NULL, NULL, NULL, 0); };
			LYNXFORCEINLINE void					Clear(int flag, LYNXCOLORRGBA& c, LYNXREAL z, int s) 
			{
				...
				m_lpD3DDevice->Clear(0, NULL, Flag, D3DCOLOR_RGBA(c.s.Red, c.s.Green, c.s.Blue, c.s.Alpha), z, (DWORD)s);					
			}			
			//Fixed functions ----------------------------------------------------------------------------------------------------------------------------------------
			...
			LYNXFORCEINLINE FIXEDVERTEXSHADER		GetFixedVertexShader() { return  m_FixedVertexShader; };
			LYNXFORCEINLINE void					SetFixedVertexShader(FIXEDVERTEXSHADER vs) {m_lpD3DDevice->SetFVF(vs); };
			...
			void									GetViewportRect(VIEWPORT* lpvp);			
			void									SetScissorRect(int x1, int y1, int x2, int y2);
			void									GetScissorRect(LPLYNXRECT lpsr);
			LYNXFORCEINLINE void					EnableScissorRect(LYNXBOOL b) {m_lpD3DDevice->SetRenderState(D3DRS_SCISSORTESTENABLE, b); };
			...
			{ 
				...
				if (lpb)
					m_lpD3DDevice->SetRenderTarget(index, ((CD3D9Buffer*)((CBuffer*)lpb))->GetlpSurface());  	
				else
			...
			{ 
				...
				m_CachedDB = lpzb;
				if (lpzb)
					m_lpD3DDevice->SetDepthStencilSurface(((CD3D9Buffer*)((CBuffer*)lpzb))->GetlpSurface());  	
				else
			...
			LYNXFORCEINLINE virtual CVertexShader*	CreateVertexShader() {return LYNXNEW CD3D9VertexShader(m_lpGraphicsSystem); };
			LYNXFORCEINLINE virtual CPixelShader*	CreatePixelShader() {return LYNXNEW CD3D9PixelShader(m_lpGraphicsSystem); };

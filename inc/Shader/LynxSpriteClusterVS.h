//###########################################################################
//    __                    ____            __
//   / /  __ __ ___ _  __  / __/___  ____  /_/___  ____
//  / /__/ // /  _ \ \/ / / __/  _ \/ __ \/ /  _ \/  _/
//  \___/__, /__//_/_/\_\ \__/__//_/\__, /_/__//_/\__/
//      /___/                       /___/ 
// 
//  LynxEngine Ver 1.00 
//  Copyright (C) 2003 fallingCAT studios.  All Rights Reserved.
//
//
//  Created by Owen Wu : 2005/11/02
//  Last Update : 
//--------------------------------------------------------------------------
//  說明:
//
//###########################################################################

#ifndef __LYNXSPRITECLUSTERVS_H__
#define __LYNXSPRITECLUSTERVS_H__

#include <GraphicsSystem/LynxShaderClass.h>

namespace LynxEngine 
{	
	namespace Shader 
	{		
		class LYNXENGCLASS CSpriteClusterVS : public GraphicsSystem::CShaderClass
		{	
			LYNX_DECLARE_CLASS(CSpriteClusterVS);
		public:						
		protected:					
			GraphicsSystem::CShaderConstant*	m_WorldViewProjM;
			GraphicsSystem::CShaderConstant*    m_PositionArray;
            GraphicsSystem::CShaderConstant*    m_ColorArray;
            GraphicsSystem::CShaderConstant*    m_SizeArray;			
		public:			
			CSpriteClusterVS(GraphicsSystem::CGraphicsSystem* lpg);
			virtual ~CSpriteClusterVS();

			virtual LYNXBOOL					vCreate();
			virtual void						vCreateConstantAndResource();
			virtual void						vSet(CContainer* lpc, CCameraContainer* cam);

			LYNXFORCEINLINE GraphicsSystem::CShaderConstant* GetPositionArrayConstant() {return m_PositionArray; };
			LYNXFORCEINLINE GraphicsSystem::CShaderConstant* GetColorArrayConstant() {return m_ColorArray; };
            LYNXFORCEINLINE GraphicsSystem::CShaderConstant* GetSizeArrayConstant() {return m_SizeArray; };			
		};		
	}		
}

#endif
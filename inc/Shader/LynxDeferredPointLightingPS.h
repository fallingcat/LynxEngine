...
#ifndef __LYNXDEFERREDPOINTLIGHTINGPS_H__
#define __LYNXDEFERREDPOINTLIGHTINGPS_H__
...
{	
	namespace Shader 
	{		
		class LYNXENGCLASS CDeferredPointLightingPS : public GraphicsSystem::CShaderClass
		{	
			LYNX_DECLARE_CLASS(CDeferredPointLightingPS);
		public:		
			typedef struct {
				float			Color[3];    
				float			Intensity;					
			...
			typedef struct {
				...
				BOOL			bShadowing;
				BOOL			bHDRRendering;
				BOOL			bProjectedLightMap;
				LYNXMATRIX		ViewToWorld;
				LYNXMATRIX		LightViewProj;	
				float			Exposure;								
			}PARAMDATA;
		protected:				
		...
		public:			
			CDeferredPointLightingPS(GraphicsSystem::CGraphicsSystem* lpg);
			virtual ~CDeferredPointLightingPS();

			LYNXBOOL							vCreate();
			void								vCreateConstantAndResource();			
			void								vBindResource();
			void								UpdateConstants(CContainer* lpc, CCameraContainer* cam);
			void								SetResources(CContainer* lpc, CCameraContainer* cam);

...
{	
	namespace Shader
	{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		CSkinningModelVS::CSkinningModelVS(GraphicsSystem::CGraphicsSystem *pgs)
		: CShaderClass(pgs)
		{	
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		CSkinningModelVS::~CSkinningModelVS(void)
		{
			if (m_WorldViewProjM)
			...
			if (m_BoneMatrix)
				m_lpGraphicsSystem->GetlpRenderer()->ReleaseShaderConstant(m_BoneMatrix);
		}				
		//-------------------------------------------------------------------------------------------------------
		//
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CSkinningModelVS::vCreateConstantAndResource() 
		{ 
			if (!m_WorldViewProjM)
			...
			if (!m_BoneMatrix)
				m_BoneMatrix = m_lpGraphicsSystem->GetlpRenderer()->CreateShaderConstant(m_lpShader, "gBoneMatrix");
		}	
		//-------------------------------------------------------------------------------------------------------
		//
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL CSkinningModelVS::vCreate() 
		{ 
			return m_lpGraphicsSystem->GetlpRenderer()->LoadVertexShader(m_lpShader, _T("model/lynxMBS.vso"), &m_ShaderMacros);
		}
		//-------------------------------------------------------------------------------------------------------
		//
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CSkinningModelVS::vSet(CContainer* lpc, CCameraContainer* cam) 
		{ 

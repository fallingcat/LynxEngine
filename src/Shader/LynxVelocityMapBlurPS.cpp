	...
	namespace Shader
	{
		...
		{ 
            ...
            m_ShaderMacros.clear();
				...
				ShaderMacro.Name = CAnsiString("NUM_SAMPLES");
				ShaderMacro.Definition = CAnsiString("16");
				m_ShaderMacros.push_back(ShaderMacro);
				//m_Distance = 0.5f;
			#endif
			
			return m_lpGraphicsSystem->GetlpRenderer()->LoadPixelShader(m_lpShader, _T("common/lynxVelocityMapBlur.pso"), &m_ShaderMacros);
		}
		//-------------------------------------------------------------------------------------------------------
		//
		//  說明:   

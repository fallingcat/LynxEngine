...
{	
	namespace Shader
	{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		CSpriteGrassFadeOutDirLightingPS::CSpriteGrassFadeOutDirLightingPS(GraphicsSystem::CGraphicsSystem *pgs)
		: CShaderClass(pgs)
		{	
		}
		//-------------------------------------------------------------------------------------------------------
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		CSpriteGrassFadeOutDirLightingPS::~CSpriteGrassFadeOutDirLightingPS(void)
		{
		}						
		//-------------------------------------------------------------------------------------------------------
		//
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL CSpriteGrassFadeOutDirLightingPS::vCreate() 
		{ 
			return m_lpGraphicsSystem->GetlpRenderer()->LoadPixelShader(m_lpShader, _T("scn/spritegrass_fadeout_directionallight.pso"), &m_ShaderMacros);
		}		
		//-------------------------------------------------------------------------------------------------------
		//
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CSpriteGrassFadeOutDirLightingPS::vSet(CContainer* lpc, CCameraContainer* cam) 
		{ 
			...
			SceneSystem::CDirectionalLight* lpDL = (SceneSystem::CDirectionalLight*)m_lpGraphicsSystem->GetlpMaterialSystem()->GetlpCurrentLight();
			if (lpc->m_bReceiveShadow)
			{
				m_lpShader->vSetTexture(MaterialSystem::CMaterialSystem::GetMapString(MaterialSystem::SHADOWMAP).c_str(), lpDL->GetShadowMap().GetCurrent());												
				...
				TexelGrad[0] = 1.0f/(float)(lpDL->GetShadowMap().GetWidth());
				TexelGrad[1] = 1.0f/(float)(lpDL->GetShadowMap().GetHeight());
				m_lpShader->vSetConstantF("gShadowmapTexelGrad", TexelGrad, 2);
			}
			Color[0] = lpDL->m_Color.Red/255.0f;
			Color[1] = lpDL->m_Color.Green/255.0f;
			Color[2] = lpDL->m_Color.Blue/255.0f;
			m_lpShader->vSetConstantF("gLight.Color", Color, 3);
			m_lpShader->vSetConstantF("gLight.Intensity", lpDL->m_Intensity);
			m_lpShader->vSetConstantF("gLight.Radius", lpDL->m_Radius);
			lpDL->GetDirection(Pos);
			m_lpShader->vSetConstantF("gLight.Dir", &Pos.x, 3);						
			m_lpShader->vSetConstantF("gZBias", lpDL->m_DepthBias);
			m_lpShader->vSetConstantI("gShadowFilterType", lpDL->m_ShadowFilterType);

...
{	
	namespace Shader
	{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		CHaloRadialBlurPS::CHaloRadialBlurPS(GraphicsSystem::CGraphicsSystem *pgs)
		: CShaderClass(pgs)
		{	
			m_RadialCenter_Const = NULL;
			m_HaloRadius_Const = NULL;
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		CHaloRadialBlurPS::~CHaloRadialBlurPS(void)
		{
			if (m_RadialCenter_Const)
				m_lpGraphicsSystem->GetlpRenderer()->ReleaseShaderConstant(m_RadialCenter_Const);
			if (m_HaloRadius_Const)
				m_lpGraphicsSystem->GetlpRenderer()->ReleaseShaderConstant(m_HaloRadius_Const);
		}						
		//-------------------------------------------------------------------------------------------------------
		//
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL CHaloRadialBlurPS::vCreate() 
		{ 
			...
			return m_lpGraphicsSystem->GetlpRenderer()->LoadPixelShader(m_lpShader, _T("common/lynxHaloRadialBlur.pso"), &m_ShaderMacros);
		}
		//-------------------------------------------------------------------------------------------------------
		//
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CHaloRadialBlurPS::vCreateConstantAndResource() 
		{ 
			if (!m_RadialCenter_Const)
				m_RadialCenter_Const = m_lpGraphicsSystem->GetlpRenderer()->CreateShaderConstant(m_lpShader, "gRadialCenter");
			if (!m_HaloRadius_Const)
				m_HaloRadius_Const = m_lpGraphicsSystem->GetlpRenderer()->CreateShaderConstant(m_lpShader, "gHaloRadius");
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CHaloRadialBlurPS::vSet(CContainer* lpc, CCameraContainer* cam) 
		{ 			
			...
			m_RadialCenter_Const->vMap();
				...
				m_HaloRadius_Const->vSetFloat(m_HaloRadius);

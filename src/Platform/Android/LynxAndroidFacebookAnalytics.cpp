...
namespace LynxEngine
{
    namespace Android
	{
		...
		void CFacebookAnalytics::Config(const CString& build)
		{
			...
			jclass cls = FindClass(env, "LynxEngine/Android/LynxFacebookAnalytics");
		...
		void CFacebookAnalytics::Init()
		{
			...
			jmethodID setuserid = env->GetStaticMethodID(cls, "SetUserID", "(Ljava/lang/String;)V");			
			jstring n = NewModifiedUTF8String(env, glpKernel->GetUDIDString().c_str());
			env->CallStaticVoidMethod(cls, setuserid, n);
		...
		// 
		//-------------------------------------------------------------------------------------------------------
		//void CFacebookAnalytics::AddDesignEvent(const CString& eventId)
		//{
		//	JNI_ATTACH_CURRENT_THREAD
		...
		//	
		//-------------------------------------------------------------------------------------------------------
		void CFacebookAnalytics::AddBusinessEvent(const CString& currency,
											  ...
											  const CString& stack)
		{
			...
			jstring jstack = NewModifiedUTF8String(env, stack.c_str());
		...
		{
			LYNX_LOG(glpLogger, CString(_T("source CFacebookAnalytics::UpdateUserProperty")));
			JNI_ATTACH_CURRENT_THREAD

	...
	{	
		namespace Android
		{
			...
			void CGraphicsSystem::vLoadConfig(const LYNXCHAR* filename)
			{
				...
				Format = IniReader.ReadString(_T("Graphics"), _T("ShadowMapFormat"), LynxEngine::GetEnumStr(CRenderer::FORMAT_A8R8G8B8).c_str());
			...
			//  說明:   
			//-------------------------------------------------------------------------------------------------------
			LYNXBOOL CGraphicsSystem::vInit(LYNXWINDOW wnd, const int w, const int h)
			{	
				LYNX_GUARD(CGraphicsSystem::vInit)
			...
			{	
				...
				LYNX_LOG(glpLogger, CString(GetClassName())+CString(_T("::InitDefalutResources...\r\n")));
				...
				m_lpRenderer->LoadTexture(m_SystemTextures[WHITE_TEXTURE], *(m_lpEngine->GetlpFileSystem()->GetPackage()->GetStream()), _T("../texture/white.png"), &TexDesc);
				m_lpRenderer->LoadTexture(m_SystemTextures[BLACK_TEXTURE], *(m_lpEngine->GetlpFileSystem()->GetPackage()->GetStream()), _T("../texture/black.png"), &TexDesc);				
				...
				LYNX_LOG(glpLogger, CString(GetClassName())+CString(_T("::InitDefalutResources OK\r\n")));
			...
			{				
				...
				if (m_lpMaps[OFFSCREEN])
				{
                    ...
                    switch (m_lpEngine->GetlpKernel()->GetDeviceType())
                    {
                        ...
                        case DT_IPAD_1:
							...
							if (!((COffscreen*)m_lpMaps[OFFSCREEN])->Create(1, _T("Offscreen"), cx, cy, CRenderer::FORMAT_A8R8G8B8, 2, OffscreenShift))
                                return LYNX_FALSE;
                            break;
                         
                        default:
                            OffscreenShift = LYNX_CLAMP((3-m_Config.PostprocessQuality), 1, 3);
							if (!((COffscreen*)m_lpMaps[OFFSCREEN])->Create(1, _T("Offscreen"), cx, cy, CRenderer::FORMAT_A8R8G8B8, 2, OffscreenShift))
                                return LYNX_FALSE;
                            break;
                    }
				...
				if (m_lpMaps[UI_TRANSITION])
				{
					...
					if (!((COffscreen*)m_lpMaps[UI_TRANSITION])->Create(1, _T("UI Transition"), cx, cy, CRenderer::FORMAT_A8R8G8B8, 0, 0))				
						return LYNX_FALSE;				

...
{	
	namespace InputSystem 
	{
		...
		{			
			...
			: LynxEngine::InputSystem::CTeslameter(lpsys)
			{
			...
			{			
				CLLocationManager* LocationManager = static_cast<LynxEngine::GameFramework::iOS::CKernel*>(m_lpInputSystem->GetlpEngine()->GetlpKernel())->GetLocationManager();
                if (LocationManager != nil)
                    m_Value = LynxEngine::Math::CVector3(LocationManager.heading.x, LocationManager.heading.y, LocationManager.heading.z);                
			}
		}
	}
}
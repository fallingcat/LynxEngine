    ...
    */
	...
	{		
		...
		CEngine::CEngine(void)
		{
			...
			m_ConfigFilename = _T("../config/ios_engine.ini");
		}
		//-------------------------------------------------------------------------------------------------------
		//
		//  說明:   
		...
		LYNXBOOL CEngine::vCreateSubSystems(GameFramework::CKernel::INIT_DESC* desc)	
		{
			...
			m_lpFileSystem = LYNXNEW FileSystem::iOS::CFileSystem(this);
            if (m_lpFileSystem)
            {
			...
			m_lpGraphicsSystem = LYNXNEW GraphicsSystem::iOS::CGraphicsSystem(this); 
			if (desc)
			{
		...
		{
			m_bLockFrameRate = LYNX_TRUE;
			m_UpdateRate = 60.0f;		
			
			if (m_lpPhysicsSystem)
			{
				m_lpPhysicsSystem->m_UpdateRate = 20.0f;
			}
		}
        */
		//-------------------------------------------------------------------------------------------------------
		...
		{			
			...
			if (m_lpInputSystem)
			{
				...
				m_lpSystemAccelerometer = (LynxEngine::InputSystem::CAccelerometer *)m_lpInputSystem->CreateDevice(InputSystem::ACCELEROMETER);
                m_lpSystemAccelerometer->SetName(_T("System Accelerometer"));
                m_lpSystemGyrometer = (LynxEngine::InputSystem::CGyrometer *)m_lpInputSystem->CreateDevice(InputSystem::GYROMETER);
                m_lpSystemGyrometer->SetName(_T("System Gyrometer"));

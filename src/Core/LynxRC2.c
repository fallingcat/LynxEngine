...
void rc2_keyschedule( unsigned short xkey[64],
	...
	LYNX_ASSERT(len > 0 && len <= 128);
	LYNX_ASSERT(bits <= 1024);
	if (!bits)
		bits = 1024;
	...
	len = (bits+7) >> 3;
	i = 128-len;
	x = permute[((unsigned char *)xkey)[i] & (255 >> (7 & -((signed)bits)))];
...
int lynxRC2Encrypt(char *cipher, const char* key, const char *plain)
{
	unsigned char *KeyStr = NULL;
	unsigned short Key[64];
	unsigned char Block[8];
	unsigned char CipherBlock[8];
	int Len = 0, PlainLen = 0, PlainOffset = 0;

	Len = strlen(key);
	KeyStr = (unsigned char *)key;
	rc2_keyschedule(Key, KeyStr, Len, 1024);
	...
	while (PlainLen)
	{
		if (PlainLen >= 8)
		{
			lynxMemCpy(Block, &plain[PlainOffset], 8);
			rc2_encrypt(Key, Block, CipherBlock);
			lynxMemCpy(&cipher[PlainOffset], CipherBlock, 8);
			PlainOffset += 8;
			PlainLen -= 8;
		}
		else
		{
			lynxMemCpy(Block, &plain[PlainOffset], PlainLen);
			lynxMemSet(&Block[PlainLen], 0x0, 8-PlainLen);
			rc2_encrypt(Key, Block, CipherBlock);
			lynxMemCpy(&cipher[PlainOffset], CipherBlock, 8);
...
void lynxRC2Decrypt(char *plain, const char* key, const char *cipher, const int len)
{
	unsigned char *KeyStr = NULL;
	unsigned short Key[64];
	unsigned char Block[8];
	unsigned char CipherBlock[8];
	int Len = 0, CipherLen = 0, CipherOffset = 0;
	...
	while (CipherLen)
	{
		lynxMemCpy(CipherBlock, &cipher[CipherOffset], 8);
		rc2_decrypt(Key, Block, CipherBlock);
		lynxMemCpy(&plain[CipherOffset], Block, 8);
		CipherOffset += 8;
		CipherLen -= 8;		
	}
	plain[CipherOffset] = 0x0;
}

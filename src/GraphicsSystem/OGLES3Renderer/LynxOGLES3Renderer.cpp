...
namespace LynxEngine
{
	namespace GraphicsSystem
	{
		...
		//  說明:
		//-------------------------------------------------------------------------------------------------------
		int COGLES3Renderer::GetIndexCount(const CRenderer::PRIMITIVETYPE type, const int primitive_count)
		{
			switch (type)
			{
		...
		//  說明:    
		//-------------------------------------------------------------------------------------------------------
		GLenum COGLES3Renderer::TranslatePrimitiveType(CRenderer::PRIMITIVETYPE e)
		{
			GLenum PType;
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		GLint COGLES3Renderer::TranslateAddressMode(CRenderer::ADDRESSMODE e)
		{
			switch (e)
			{
				default:
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		GLenum	COGLES3Renderer::TranslateCullMode(CRenderer::CULLMODE e)
		{
			switch (e)
			{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		GLenum COGLES3Renderer::TranslateTestFunction(CRenderer::TESTFUNC e)
		{
			switch (e)
			{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		GLenum COGLES3Renderer::TranslateStencilOP(CRenderer::STENCILOP e)
		{
			switch (e)
			{
				default:
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		GLenum COGLES3Renderer::TranslateBlendFunction(CRenderer::BLENDFUNC e)
		{
			switch (e)
			{
				case BF_ZERO:
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		GLuint COGLES3Renderer::TranslateVertexSlotType(CRenderer::VERTEXSLOTTYPE e)
		{
			switch (e)
			{
				default:					
		...
		*/
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		GLenum COGLES3Renderer::TranslateVertexSlotFormat(CRenderer::VERTEXSLOTFORMAT e)
		{
			switch (e)
			{
				default:
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		GLenum COGLES3Renderer::TranslateFormat(CRenderer::FORMAT f)
		{
			switch (f)
			{
				default:
				...
				case CRenderer::FORMAT_RGB8_ETC2:
					return GL_COMPRESSED_RGB8_ETC2;
				case CRenderer::FORMAT_RGBA8_ETC2:
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		GLenum COGLES3Renderer::TranslateFormatToPixelType(CRenderer::FORMAT f)
		{
			switch (f)
			{
				case CRenderer::FORMAT_R5G6B5:
		...
		//  說明:    
		//-------------------------------------------------------------------------------------------------------
		COGLES3Renderer::COGLES3Renderer() 
		{
			m_RendererType = OGLES3_RENDERER;
			m_FrameBuffer = 0;
            m_R2TFrameBuffer = 0;
			m_MSFrameBuffer = 0;

			m_bSharedShaderGroup = LYNX_FALSE;
		...
		//  說明:    
		//-------------------------------------------------------------------------------------------------------
		COGLES3Renderer::COGLES3Renderer(CGraphicsSystem* lpgs)
		: CRenderer(lpgs) 
		{			
			m_RendererType = OGLES3_RENDERER;
			m_FrameBuffer = 0;
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		CString& COGLES3Renderer::ComputeCachedCompiledShaderPath() 
		{
			CString				Target;
			LYNXCHAR			Buffer[16];
			...
			#ifdef __iOS__
				m_ShaderSrcPath = CString(_T("engine/shader/ogles/"));
		...
		//  說明:    
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL COGLES3Renderer::Open(LYNXWINDOW wnd, INITGRAPHICPARAM* lpcp)
		{
			...
			GLenum DepthFormat = TranslateFormat(lpcp->DepthBufferFormat);
                    ...
                    ((COGLES3Buffer*)((CBuffer*)m_lpBackBuffer))->SetGLHandle(GLHandle);
                    ...
                    ((COGLES3Buffer*)((CBuffer*)m_lpDepthBuffer))->SetGLHandle(GLHandle);
                #else
                    if (lpcp->NumMultiSamples <= 1)
                    {
                        ...
                        m_lpBackBuffer->vCreate(m_Window, lpcp->Width, lpcp->Height, lpcp->FrameBufferFormat, 0, LYNX_FALSE);
                        m_lpBackBuffer->SetName(_T("SystemBackBuffer"));
                        ((COGLES3Buffer*)((CBuffer*)m_lpBackBuffer))->SetGLHandle(ColorBuffer);
						((COGLES3Buffer*)((CBuffer*)m_lpBackBuffer))->SetFrameBuffer(m_FrameBuffer);
                        
                        m_lpGraphicsSystem->GetlpEngine()->CreateResource(m_lpDepthBuffer, LynxEngine::BUFFER);
                        m_lpDepthBuffer->vCreate(m_Window, lpcp->Width, lpcp->Height, lpcp->DepthBufferFormat, 0, LYNX_FALSE);
                        m_lpDepthBuffer->SetName(_T("SystemDepthBuffer"));
                        glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, ((COGLES3Buffer*)((CBuffer*)m_lpDepthBuffer))->GetGLHandle());
                        // Attach to stencil
                        if (lpcp->DepthBufferFormat == CRenderer::FORMAT_D24S8)
                        {
                            glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, ((COGLES3Buffer*)((CBuffer*)m_lpDepthBuffer))->GetGLHandle());
                        }
                    ...
                    else 
                    {
                        ...
                        ((COGLES3Buffer*)((CBuffer*)m_lpResolvedBackBuffer))->SetGLHandle(ColorBuffer);
                        ...
                        m_lpBackBuffer->SetName(_T("SystemBackBuffer"));
                        ((COGLES3Buffer*)((CBuffer*)m_lpBackBuffer))->SetGLHandle(MSColorBuffer);
                        ((COGLES3Buffer*)((CBuffer*)m_lpBackBuffer))->SetFrameBuffer(m_FrameBuffer);
                        ...
                        m_lpDepthBuffer->SetName(_T("SystemDepthBuffer"));
                        ((COGLES3Buffer*)((CBuffer*)m_lpDepthBuffer))->SetGLHandle(MSDepthBuffer);
				...
				*/
				...
				#ifdef __ANDROID__
					ANativeWindow_setBuffersGeometry(m_Window, 0, 0, NativeVid);
				#endif
                ...
                m_lpGraphicsSystem->GetlpEngine()->CreateResource(m_lpBackBuffer, LynxEngine::BUFFER);
                m_lpBackBuffer->SetName(_T("SystemBackBuffer"));
                ((COGLES3Buffer*)((CBuffer*)m_lpBackBuffer))->SetGLHandle(0);
				((COGLES3Buffer*)((CBuffer*)m_lpBackBuffer))->SetFrameBuffer(0);
                ((COGLES3Buffer*)((CBuffer*)m_lpBackBuffer))->SetInfo(lpcp->Width, lpcp->Height, lpcp->FrameBufferFormat);
                ...
                //glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME, &GLHandle);
					...
					m_lpGraphicsSystem->GetlpEngine()->CreateResource(m_lpDepthBuffer, LynxEngine::BUFFER);
					m_lpDepthBuffer->SetName(_T("SystemDepthBuffer"));
					((COGLES3Buffer*)((CBuffer*)m_lpDepthBuffer))->SetGLHandle(0);
					((COGLES3Buffer*)((CBuffer*)m_lpDepthBuffer))->SetFrameBuffer(0);
					((COGLES3Buffer*)((CBuffer*)m_lpDepthBuffer))->SetInfo(lpcp->Width, lpcp->Height, lpcp->DepthBufferFormat);
					...
					m_lpDepthBuffer->vCreate(m_Window, lpcp->Width, lpcp->Height, lpcp->DepthBufferFormat, 0, LYNX_FALSE);
					m_lpDepthBuffer->SetName(_T("SystemDepthBuffer"));
					GL_TRACE(glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, ((COGLES3Buffer*)((CBuffer*)m_lpDepthBuffer))->GetGLHandle());)
					// Attach to stencil
					if (lpcp->DepthBufferFormat == CRenderer::FORMAT_D24S8)
					{
						GL_TRACE(glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, ((COGLES3Buffer*)((CBuffer*)m_lpDepthBuffer))->GetGLHandle());)
					}
				#endif
				...
				m_lpDepthBuffer->vCreate(m_Window, lpcp->Width, lpcp->Height, lpcp->DepthBufferFormat, 0, LYNX_FALSE);
				m_lpDepthBuffer->SetName(_T("SystemDepthBuffer"));
				((COGLES3Buffer*)((CBuffer*)m_lpDepthBuffer))->SetGLHandle(DepthBuffer);
		...
		//  說明:    
		//-------------------------------------------------------------------------------------------------------
		void COGLES3Renderer::Clear(int flag, LYNXCOLORRGBA& c, LYNXREAL z, int s)
		{		
			GLbitfield Flag = 0;
		...
		//  說明:    
		//-------------------------------------------------------------------------------------------------------
		void COGLES3Renderer::DrawElementsBO(PRIMITIVETYPE type, int base_vert, DWORD num_vert, int base_index, DWORD primitive_count) 
		{
			START_COUNTER
			
			GL_TRACE(glDrawElements(TranslatePrimitiveType(type), GetIndexCount(type, primitive_count), 
						   ((COGLES3IndexArray*)m_CachedIndexArray)->GetIndexType(), 
		...
		//  說明:    
		//-------------------------------------------------------------------------------------------------------
		void COGLES3Renderer::DrawPrimitiveBO(PRIMITIVE* lpp, DWORD num_vert)
		{
			START_COUNTER
		...
		//  說明:    
		//-------------------------------------------------------------------------------------------------------
		void COGLES3Renderer::DrawArrayBO(PRIMITIVETYPE type, DWORD primitive_count, int base_vert)
		{
			START_COUNTER
		...
		//  說明:    
		//-------------------------------------------------------------------------------------------------------
		void COGLES3Renderer::SwapBackBuffer()
		{
			#ifdef __iOS__
				if (m_lpResolvedBackBuffer)
				{
					...
					//SetRenderBuffer
					GL_TRACE(glBindRenderbuffer(GL_RENDERBUFFER, ((COGLES3Buffer*)((CBuffer*)m_lpResolvedBackBuffer))->GetGLHandle());)
		...
		//  說明:    
		//-------------------------------------------------------------------------------------------------------
		void COGLES3Renderer::BltBuffer(CBuffer* lpdest, LYNXRECT* drect, CBuffer* lpsrc, LYNXRECT* srect)
        {
		...
		//-------------------------------------------------------------------------------------------------------
		void COGLES3Renderer::DrawScreenQuad(void* lpv, DWORD size)
        {

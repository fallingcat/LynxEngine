...
{	
	namespace GraphicsSystem 
	{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		COGLES3Texture::COGLES3Texture(CGraphicsSystem *pgs)
		: CTexture(pgs)
		{	
			m_GLHandle = 0;
		}
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		COGLES3Texture::~COGLES3Texture(void)
		{
			if (m_GLHandle)
			{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void COGLES3Texture::vGetLevel(int face, int level)
		{
		}
		//-------------------------------------------------------------------------------------------------------
		//
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL COGLES3Texture::LoadTextureBMP(CStream& fs, const LYNXCHAR* name, int mipmap, CRenderer::TEXTURETYPE type)
		{
			return LYNX_FALSE;
		}
		//-------------------------------------------------------------------------------------------------------
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void COGLES3Texture::LoadPVR(CStream& fs, PVRHEADER* lpheader)
		{
			...
			while (DataOffset < DataLength)
			{
				...
				lpData = LYNXCORE_NEW char[DataSize];
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL COGLES3Texture::LoadTexturePVR(CStream& fs, const LYNXCHAR* name, int mipmap, CRenderer::TEXTURETYPE type)
		{
			PVRHEADER			PVRHeader;
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL COGLES3Texture::LoadTextureKTX(CStream& fs, const LYNXCHAR* name, int mipmap, CRenderer::TEXTURETYPE type)
		{
			int size = fs.vSize();
			void* bytes = LYNXCORE_NEW BYTE[size];
			...
			if (!Create(name, dim.width, dim.height, mipmap, type, CRenderer::FORMAT_RGBA8_ETC2, m_GLHandle))
			{
				return LYNX_FALSE;
			}
			
			LYNXCORE_DEL_ARRAY(bytes);
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void COGLES3Texture::LoadTGA(CStream& fs, TGAHEADER* lpheader)
		{
			BYTE                *lpB;
			DWORD               *lpDW;
			WORD                *lpW;
			...
			switch (m_lpSrcBuffer->GetFormat())
			{
				case CRenderer::FORMAT_A8R8G8B8:	
					lpSrc = (BYTE *)LYNXCORE_NEW BYTE[lpheader->imwidth*lpheader->imheight*4];
					if (lpheader->imdepth == 24)
					{		
						lpOMem = lpMem = (BYTE *)LYNXCORE_NEW BYTE[lpheader->imwidth*lpheader->imheight*4];
				...
				case CRenderer::FORMAT_R8G8B8:	
					...
					if (lpheader->imdepth == 24)
					{
						lpOMem = lpMem = (BYTE *)LYNXCORE_NEW BYTE[lpheader->imwidth*lpheader->imheight*3];
						fs.vRead(lpMem, sizeof(BYTE), lpheader->imwidth*lpheader->imheight*3);										
						if (lpheader->imdesc & 0x20)
						{
							for (y=0; y<lpheader->imheight; y++)
			...
			LYNXCORE_DEL_ARRAY(lpSrc);			
		}
		//-------------------------------------------------------------------------------------------------------
		//
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL COGLES3Texture::LoadTextureTGA(CStream& fs, const LYNXCHAR* name, int mipmap, CRenderer::TEXTURETYPE type)
		{
			TGAHEADER			TGAHeader;
			CRenderer::FORMAT	Format;		
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void COGLES3Texture::LoadPNG(BYTE* data, png_infop info)
		{
			...
			switch (m_lpSrcBuffer->GetFormat())
			{
				case CRenderer::FORMAT_A8R8G8B8:	
					BPP = 4;
					lpSrc = (BYTE *)LYNXCORE_NEW BYTE[info->width*info->height*4];
				...
				case CRenderer::FORMAT_R8G8B8:	
					BPP = 3;
					lpSrc = (BYTE *)LYNXCORE_NEW BYTE[info->width*info->height*3];
					if (info->color_type == PNG_COLOR_TYPE_RGB)
					{		
						for (y=0; y<info->height; y++)
						{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL COGLES3Texture::LoadTexturePNG(CStream& fs, const LYNXCHAR* name, int mipmap, CRenderer::TEXTURETYPE type)
		{
			CRenderer::FORMAT	Format;
			...
			if (!m_bDelayLoading)
			{
				...
				BYTE* data = LYNXCORE_NEW BYTE[row_bytes * Height];           
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL COGLES3Texture::DelayLoadTexturePNG(CStream& fs)
		{
			CRenderer::FORMAT	Format;
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL COGLES3Texture::vLoad(CStream& fs, const LYNXCHAR *filename, CRenderer::TEXTUREDESC* desc)
		{
            ...
            if (desc && desc->bLoadFromMemory && desc->Width > 0 && desc->Height > 0)
            {
                ...
                {                   
					GL_TRACE(glTexImage2D(GL_TEXTURE_2D, 0, COGLES3Renderer::TranslateFormat(desc->TargetFormat), desc->Width, desc->Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, desc->lpData);)
                }
            }
            else
            {
                ...
                if (!lynxStriCmp(Ext, _T("bmp")))	
                    bLoadTexture = LoadTextureBMP(fs, m_FullPath.c_str(), m_MipMapLevel, m_TextureType);
			    else if (!lynxStriCmp(Ext, _T("png")))
                    bLoadTexture = LoadTexturePNG(fs, m_FullPath.c_str(), m_MipMapLevel, m_TextureType);
                else if (!lynxStriCmp(Ext, _T("tga")))
                    bLoadTexture = LoadTextureTGA(fs, m_FullPath.c_str(), m_MipMapLevel, m_TextureType);
                else if (!lynxStriCmp(Ext, _T("pvr")))
                    bLoadTexture = LoadTexturePVR(fs, m_FullPath.c_str(), m_MipMapLevel, m_TextureType);
				else if (!lynxStriCmp(Ext, _T("ktx")))
                    bLoadTexture = LoadTextureKTX(fs, m_FullPath.c_str(), m_MipMapLevel, m_TextureType);
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL COGLES3Texture::Create(const LYNXCHAR *name, int w, int h, int mipmaplevel, CRenderer::TEXTURETYPE type, CRenderer::FORMAT format, GLuint handle)
		{
			DWORD		NumMip;
			DWORD		Usage;			
			BYTE*		lpB;
			...
			((COGLES3Buffer*)&(*m_lpSrcBuffer))->SetBufferType(COGLES3Buffer::BT_TEXTURE2D);
			
			m_PixelFormat = COGLES3Renderer::TranslateFormat(format);
			...
			m_Height = h;
			SetName(name);
			((COGLES3Buffer*)&(*m_lpSrcBuffer))->SetGLHandle(m_GLHandle);
            ((COGLES3Buffer*)&(*m_lpSrcBuffer))->SetlpTexture(this);
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL COGLES3Texture::vCreate(const LYNXCHAR *name, int w, int h, int mipmaplevel, CRenderer::TEXTURETYPE type, CRenderer::FORMAT format)
		{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL COGLES3Texture::vCreateRT(CBufferPtr* lpzb, CRenderer::FORMAT zfmt, const LYNXCHAR *name, int w, int h, int mipmaplevel, CRenderer::TEXTURETYPE type, CRenderer::FORMAT format, int ms, LYNXBOOL blockable)
		{
			DWORD							NumMip;
			DWORD							Usage;
			int								i, j;
			...
			m_PixelFormat = COGLES3Renderer::TranslateFormat(format);
			GLenum PixelType = COGLES3Renderer::TranslateFormatToPixelType(format);
			...
			((COGLES3Buffer*)&(*m_lpSrcBuffer))->SetGLHandle(m_GLHandle);
            ((COGLES3Buffer*)&(*m_lpSrcBuffer))->SetlpTexture(this);
            //((COGLES3Buffer*)&(*m_lpSrcBuffer))->GenerateFrameBuffer();
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void COGLES3Texture::vUpdate(const int level, const void* data)
        {
            switch (m_TextureType & 0xff)
			{
				case CRenderer::TT_CUBE_TEXTURE:
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		CBuffer* COGLES3Texture::vSetAsRT(int channel) 
		{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void COGLES3Texture::vSet(int channel) 
		{ 			

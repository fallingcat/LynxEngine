...
{	
	namespace GraphicsSystem 
	{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		CD3D11PixelShader::CD3D11PixelShader(CGraphicsSystem *pgs)
		: CPixelShader(pgs)
		{	
			m_lpPS = NULL;
			m_lpReflection = NULL;
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		CD3D11PixelShader::~CD3D11PixelShader(void)
		{
			Release();
		}				
		//-------------------------------------------------------------------------------------------------------
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::Release()
		{		
			if (m_lpPS)
			{
				m_lpPS->Release();
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::CreateConstantBuffers()
		{
			...
			for (int i=0; i<ShaderDesc.ConstantBuffers; i++)
			{
				...
				//m_lpReflection->GetVariableByName(name)->GetDesc(&m_D3DShaderVarDesc);
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		CD3D11Buffer* CD3D11PixelShader::GetConstantBuffer(const CAnsiString& name)
		{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL CD3D11PixelShader::LoadCompiledShader(CStream& fs, const LYNXCHAR *filename)
		{
			int								i;
			BYTE							*lpData;	
			DWORD							Len;		
			...
			HRESULT hRes = lpR->GetlpD3DDevice()->CreatePixelShader((void *)lpData, Len, NULL, &m_lpPS);				
			if (FAILED(hRes))
			{
				fs.vClose();
				if (lpData)
			...
			m_lpReflection->GetDesc(&ShaderDesc);
			LYNX_LOG(glpLogger, CString(GetClassName())+CString(_T("::Load Compiled Pixel Shader <"))+Path+CString(_T(">\n")));
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL CD3D11PixelShader::LoadCachedShader(CStream& fs, const LYNXCHAR *filename, const LYNXCHAR *ver, const CArray<CRenderer::SHADERMACRO>* def, LYNXBOOL brecompile, LYNXBOOL bdebug) 
		{ 
			LYNXERRORNO				Ret;
			...
			lynxSprintf(Path, _T("%sps/%s.%s"), m_lpGraphicsSystem->GetlpRenderer()->GetShaderSrcPath().c_str(), FileName, Ext);				
			lpR->m_ShaderIncludeHandler.ComputeCurrentShaderPath(Path);			
			if (!(fs.vOpen(Path, (CStream::Read|CStream::Binary))))
			{				
				lynxSprintf(Path, _T("%sps/%s.%s"), m_lpGraphicsSystem->GetlpRenderer()->GetUserShaderSrcPath().c_str(), FileName, Ext);				
				lpR->m_ShaderIncludeHandler.ComputeCurrentShaderPath(Path);			
				if (!(fs.vOpen(Path, (CStream::Read|CStream::Binary))))
				{
			...
			if (ver)
			{
				...
				{
					strcpy(Version, "ps_4_0");		
				}
				else if (!lynxStriCmp(ver, _T("5_0")))
				{
					strcpy(Version, "ps_5_0");		
				}
				else
				{
					strcpy(Version, "ps_5_0");		
			...
			if (bDebug)
			{
				Flag |= D3DCOMPILE_DEBUG;//|D3DCOMPILE_FORCE_PS_SOFTWARE_NO_OPT;				
				Flag &= ~D3DCOMPILE_OPTIMIZATION_LEVEL3;
			...
			hRes = lpR->GetlpD3DDevice()->CreatePixelShader((void*)(lpCode->GetBufferPointer()), lpCode->GetBufferSize(), NULL, &m_lpPS);	
			if (FAILED(hRes))
			{
				lpCode->Release();
				lpCode = NULL;
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL CD3D11PixelShader::vLoad(const LYNXCHAR *filename, const CArray<CRenderer::SHADERMACRO>* def, CStream* stream) 
		{ 
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL CD3D11PixelShader::vCompile(const LYNXCHAR *filename, const LYNXCHAR *ver, const CArray<CRenderer::SHADERMACRO>* def, CStream* stream) 
		{ 
			return LYNX_FALSE; 
		}		
		//-------------------------------------------------------------------------------------------------------
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSet() 
		{ 
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
			lpR->GetlpDeviceContext()->PSSetShader(m_lpPS, NULL, 0);	
			lpR->GetlpDeviceContext()->PSSetConstantBuffers(0, m_NumBoundCBs, m_D3DBufferArray);
		}		
		//-------------------------------------------------------------------------------------------------------
		//
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetTexture(const int channel, CTexture* t)  
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
			...
			if (t && t->vIsValid())
			{
				...
				lpR->GetlpDeviceContext()->PSSetShaderResources(channel, 1, ShaderRes); 
				((CD3D11SamplerState*)(&(*(t->GetSamplerState()))))->PSSet(channel);				
			}
			else
			{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetTexture(const char *name, CTexture* t)  
		{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetSampler(const char *name, CSamplerState* lps)
		{ 
			D3D11_SHADER_INPUT_BIND_DESC Desc;
			if (m_lpReflection)
			{
				m_lpReflection->GetResourceBindingDescByName(name, &Desc);
				((CD3D11SamplerState*)lps)->PSSet(Desc.BindPoint);
			}
		}
		//-------------------------------------------------------------------------------------------------------
		//
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		int CD3D11PixelShader::vGetTextureIndex(const char *name)
		{
			D3D11_SHADER_INPUT_BIND_DESC Desc;
			if (m_lpReflection)
			{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetConstantF(const char *name, const float f) 
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetConstantF(const char *name, const float *lpdata, int num) 
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetConstantI(const char *name, const int i) 
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetConstantI(const char *name, const int *lpdata, int num) 
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetConstantB(const char *name, const BOOL b) 
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetConstantB(const char *name, const BOOL *lpdata, int num) 
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetConstantV2(const char *name, const float* lpdata) 
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
			//m_lpConstantTable->SetFloat(lpR->GetlpD3DDevice(), name, f);
		}
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetConstantV2(const char *name, const float *lpdata, int num) 
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
			//m_lpConstantTable->SetFloatArray(lpR->GetlpD3DDevice(), name, lpdata, num);
		}
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetConstantV3(const char *name, const float* lpdata) 
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
			//m_lpConstantTable->SetFloat(lpR->GetlpD3DDevice(), name, f);
		}
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetConstantV3(const char *name, const float *lpdata, int num) 
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
			//m_lpConstantTable->SetFloatArray(lpR->GetlpD3DDevice(), name, lpdata, num);
		}
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetConstantV4(const char *name, const float* lpdata) 
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
			//m_lpConstantTable->SetFloat(lpR->GetlpD3DDevice(), name, f);
		}
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetConstantV4(const char *name, const float *lpdata, int num) 
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
			//m_lpConstantTable->SetFloatArray(lpR->GetlpD3DDevice(), name, lpdata, num);
		}
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetConstantM(const char *name, LPCLYNXMATRIX lpm) 
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		void CD3D11PixelShader::vSetConstantM(const char *name, LPCLYNXMATRIX lpm, int num) 
		{
			CD3D11Renderer* lpR = (CD3D11Renderer*)(m_lpGraphicsSystem->GetlpRenderer());
			//m_lpConstantTable->SetFloatArray(lpR->GetlpD3DDevice(), name, (float*)lpm, 16);
		}

...
{	
	namespace GraphicsSystem 
	{
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		CProjectedMap::CProjectedMap(CGraphicsSystem *pgs)
		: CMap(pgs)
		{
			m_Camera = LYNXNEW CCameraContainer(pgs->GetlpEngine());
		}
		//-------------------------------------------------------------------------------------------------------
		//
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		CProjectedMap::~CProjectedMap(void)
		{			
			LYNXDEL m_Camera;
		}
		//-------------------------------------------------------------------------------------------------------
		...
		//  說明:   
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL CProjectedMap::Create(CRenderer::FORMAT zformat, int mipmaplevel, const LYNXCHAR *name, int w, int h, CRenderer::FORMAT format, int utilsize, int shift)
		{			
			int				W, H;
			LYNXCHAR		Name[128];

			LYNX_LOG(glpLogger, CString(GetClassName())+CString(_T("::Create ProjectedMap...\r\n")));
			...
			m_Camera->m_Far = 500.0f;
			m_Camera->SetFOV(60.0f, 60.0f);
			m_Camera->UpdateProjectionMatrix(1.0f);
			lynxMatrixXMatrix(&TM, &m_Camera->GetViewMatrix(), &m_Camera->GetProjectionMatrix());									
			lynxMatrixXMatrix(&m_ViewProjMatrix, &TM, &ScaleM);					

			LYNX_LOG(glpLogger, CString(GetClassName())+CString(_T("::Create ProjectedMap OK.\r\n")));

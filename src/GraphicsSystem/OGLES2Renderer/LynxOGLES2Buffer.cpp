...
{	
	namespace GraphicsSystem 
	{
		...
		//  ?°?˙:   
		//-------------------------------------------------------------------------------------------------------
		COGLES2Buffer::COGLES2Buffer(CGraphicsSystem* lpsys)
		: CBuffer(lpsys) 		
		{				
			m_GLHandle = 0;
            m_Width = 0;
		...
		//-------------------------------------------------------------------------------------------------------
		LYNXBOOL COGLES2Buffer::vCreate(LYNXWINDOW wnd, DWORD w, DWORD h, GraphicsSystem::CRenderer::FORMAT format, int num_ms, LYNXBOOL blockable) 
		{
                ...
                format == CRenderer::FORMAT_D24S8)
            {
                ...
                GL_TRACE(glRenderbufferStorage(GL_RENDERBUFFER, COGLES2Renderer::TranslateFormat(format), w, h));				
            }
            m_Format = format; 
            m_Width = w;
            m_Height = h;
		...
		//
		//-------------------------------------------------------------------------------------------------------
		void COGLES2Buffer::vSetAsRenderTarget(int index)
        {
			LOCK_MUTEX
			...
			switch (m_BufferType)
            {
                case BT_RENDERBUFFER:
					...
					static_cast<COGLES2Renderer*>(m_lpGraphicsSystem->GetlpRenderer())->BindFrameBuffer();
					GL_TRACE(glBindRenderbuffer(GL_RENDERBUFFER, m_GLHandle);)
                ...
                case BT_TEXTURE2D:
					...
					static_cast<COGLES2Renderer*>(m_lpGraphicsSystem->GetlpRenderer())->BindR2TFrameBuffer();
		...
		//
		//-------------------------------------------------------------------------------------------------------
		void COGLES2Buffer::vSetAsDepthBuffer()
        {
			LOCK_MUTEX
		...
		//
		//-------------------------------------------------------------------------------------------------------
		void COGLES2Buffer::vSaveToFile(const LYNXCHAR* name, const LYNXRECT* rect)
        {
			LOCK_MUTEX
            ...
            GL_TRACE(glReadPixels(X, Y, Width, Height, GL_RGBA, GL_UNSIGNED_BYTE, data));
				...
				if (File.vOpen(AnsiName, CStream::Write|CStream::Binary))
                {
				...
				jstring str = NewModifiedUTF8String(env, AnsiName);
